...
"""
FSI-2 Benchmark — MPI-Parallelized Single-Mesh Monolithic ALE
Launch with:  mpirun -n <nprocs> python fsi2_parallel.py

GetFEM parallel (GETFEM_PARA_LEVEL=2) automatically:
  - Partitions each mesh with METIS
  - Distributes tangent matrix assembly across processes
  - Uses parallel MUMPS for the linear solve
  - Vectors (RHS, solutions) are replicated on every process
 to run it: 
...
# Only rank 0 prints verbose output; silence others
if rank == 0:
...
# Only rank 0 creates directories and log file
if rank == 0:
    os.makedirs(output_dir, exist_ok=True)

# Barrier so rank 0 creates the directory before others might need it
if HAS_MPI:
    comm.Barrier()

# Open log file only on rank 0
log_file = None
if rank == 0:
...
def log(msg=""):
    """Print to console and write to log file (rank 0 only)."""
    if rank == 0:
        print(msg)
...
log(f"MPI Parallelization: {nprocs} process(es)  [mpi4py available: {HAS_MPI}]")
...
#############
#   MESH
#   (All processes load the same mesh;
#    GetFEM auto-partitions via METIS internally)
#############
...
# ---- FLUID: CONVECTION + VISCOUS (theta) ----
...
log(f"  Linear solver:             MUMPS (parallel)")
log("")
...
# Force computation model (used on all processes for distributed asm_generic)
...
log("=" * 60)
log("Starting FSI-2 Dynamic Analysis (PARALLEL, Biharmonic)")
log("=" * 60)
...
if rank == 0:
    progress = tqdm(desc=f"FSI-2 [{nprocs} procs]", total=num_steps)
...
for step in range(num_steps):
    ...
    # ---- Force computation (ALL processes — distributed assembly) ----
    ...
    if rank == 0:
        ...
        if step % export_every == 0 or step == num_steps - 1:
            ...
            np.save(f"{output_dir}/restart_{step:06d}.npy",
                    restart_data, allow_pickle=True)
...
if rank == 0:
    ...
    if np.any(analysis_mask):
        ...
        log(f"MPI processes used: {nprocs}")
        log("")
