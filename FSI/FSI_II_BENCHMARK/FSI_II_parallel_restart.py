"""
FSI-2 Benchmark — MPI-Parallelized with Robust Restart
Launch: mpirun --allow-run-as-root -n 4 python FSI/FSI_II_BENCHMARK/FSI_1_mesh_dynamic_test2_parallel_restart.py
"""
...
##########################################
#   FSI-2 Benchmark — PARALLEL + RESTART
##########################################
...
if not is_restart:
    ...
    log(f"FSI-2 Benchmark — Fresh Start (Parallel: {nprocs} procs)")
...
if is_master:
    progress = tqdm(desc=f"FSI-2 [{nprocs} procs]", total=remaining_steps)
