"""
FSI-3 Benchmark — MPI-Parallelized with Robust Restart
Launch: mpirun --allow-run-as-root -n 4 python FSI/FSI_III_BENCHMARK/FSI_1_mesh_dynamic_test2_parallel_restart_3.py
"""
...
##########################################
#   FSI-3 Benchmark — PARALLEL + RESTART
##########################################

output_dir = "FSI/FSI_III_BENCHMARK/FSI_Benchmark_III_Results_biharmonic_mesh__local_ref"
...
# =====================================================================
#RESTART_FROM = None
RESTART_FROM = "/root/research_project/FSI/FSI_III_BENCHMARK/FSI_Benchmark_III_Results_biharmonic_mesh__local_ref/restart_005400.npy"
# =====================================================================
...
# Fluid properties (unchanged from FSI-2)
ν_fluid = 0.001
rho_fluid = 1000.0

# ============================================
# FSI-3: Structural properties
# ============================================
rho_solid = 1000.0             # FSI-2: 10000  → FSI-3: 1000
nu_solid = 0.4                 # unchanged
mu_solid = 2.0e6               # FSI-2: 0.5e6  → FSI-3: 2.0e6
E = 2 * mu_solid * (1 + nu_solid)                                     # = 5.6e6
lambda_solid = E * nu_solid / ((1 + nu_solid) * (1 - 2 * nu_solid))   # = 8.0e6

# FSI-3: Inlet velocity
U_mean = 2.0                   # FSI-2: 1.0  → FSI-3: 2.0
...
# Penalty for kinematic relation
delta = 1.0e7
...
#############
#   MESH (all processes load the same mesh)
#############

Mesh = gf.Mesh('Import', 'gmsh', 'FSI/MESH_GMSH/TF_1MESH_quads_ref2.msh')

#############
#  REGIONS (identical on every process)
#############
...
###########
#  MODEL (all processes build the same model)
###########
...
# --- Fluid: (1-theta) source terms at n ---
...
if is_restart:
    ...
    if has_individual:
        ...
        if len(saved_u_n) != expected_u_size or len(saved_v_n) != expected_v_size:
...
if not is_restart:
    ...
    log(f"FSI-3 Benchmark — Fresh Start (Parallel: {nprocs} procs)")
...
log("=" * 60)
log("Problem Parameters (FSI-3 Benchmark)")
log("=" * 60)
...
log(f"  Re = {U_mean * 2 * r / ν_fluid:.1f}")
log("")
log("  Time stepping:")
...
#########################
#  TRACKING SETUP (all processes build force model)
#########################
...
if is_restart:
    ...
    log("Starting FSI-3 Dynamic Analysis (Global Variables, Biharmonic)")
log("=" * 60)
...
if is_master:
    progress = tqdm(desc=f"FSI-3 [{nprocs} procs]", total=remaining_steps)
...
for step in range(start_step, num_steps):
    ...
    # ---- Solve (ALL processes — distributed assembly + parallel MUMPS) ----
    ...
    # ---- Extract solution (ALL processes — vectors replicated) ----
    u_sol = md.variable("u")
    v_sol = md.variable("v")
    p_sol = md.variable("p")
    w_sol = md.variable("w")
    ...
    # ---- Update previous step (ALL processes) ----
    md.set_variable("u_n", u_sol.copy())
    md.set_variable("v_n", v_sol.copy())

    # ---- Forces (ALL processes — distributed assembly) ----
...
if is_master:
    ...
    if np.any(analysis_mask):
        ...
        log("")
        log("=" * 60)
        log("FSI-3 Benchmark Final Results (Oscillatory Regime)")
        log("=" * 60)
        ...
        log("")
        log("=" * 60)
        log("Reference Values (Turek & Hron, FSI-3)")
        log("=" * 60)
        log(f"  u_x(A) = -2.69 ± 2.53 × 10^-3")
        log(f"           (computed: {ux_mean:.4e} ± {ux_amp:.4e})")
        log(f"  u_y(A) = 1.48 ± 34.38 × 10^-3")
        ...
        log(f"  f(u_y) ≈ 5.3 Hz   (computed: {freq_uy:.2f} Hz)")
        log(f"  f(u_x) ≈ 10.9 Hz")
        log("")
